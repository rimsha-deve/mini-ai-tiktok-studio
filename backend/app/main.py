...
app = FastAPI(title="TikTok Mashup Studio", version="2.0.0", lifespan=_lifespan)
...
async def generate_rest(data: dict):
    ...
    async def run():
        ...
        await AudioService().process_audio(
            ...
            background_music_tracks=audio_cfg.get("bg_tracks") or None,
        )
...
@app.get("/api/bgmusic/tracks")
async def get_bgmusic_tracks():
    """Return available background music tracks with download status."""
    from app.services.bgmusic_service import list_tracks
    return {"success": True, "tracks": list_tracks()}


@app.post("/api/bgmusic/download/{track_id}")
async def download_bgmusic_track(track_id: str):
    """Pre-download a background music track."""
    from app.services.bgmusic_service import ensure_track
    path = await ensure_track(track_id)
    return {"success": bool(path), "track_id": track_id}
...
async def upload_thumbnail(file: UploadFile = File(...)):
    ...
    dest = os.path.join(TEMP_DIR, f"custom_thumbnail{ext}")
    content = await file.read()
