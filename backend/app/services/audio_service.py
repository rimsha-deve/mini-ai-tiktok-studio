...
class AudioService:
    ...
    async def process_audio(
        ...
        background_music_tracks: Optional[List[dict]] = None,
        progress_callback=None,
    ) -> str:
        """
        Process audio with speed adjustment and optional background music.
        background_music_tracks: list of { id, volume, speed } dicts (multi-track)
        background_music_path: legacy single-track path (still supported)
        """
        ...
        # Multi-track background music (new system)
        if background_music_tracks:
            from app.services.bgmusic_service import mix_multiple_tracks
            if progress_callback:
                await progress_callback("audio", 60, f"Mixing {len(background_music_tracks)} background track(s)...")
            duration = await self._get_duration(current_audio)
            await mix_multiple_tracks(current_audio, background_music_tracks, output_path, duration)
