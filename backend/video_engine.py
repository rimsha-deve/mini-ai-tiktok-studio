...
def process_avatar(image_path: str) -> Image.Image:
    ...
    try:
        ...
        with open(image_path, "rb") as f:
            raw = f.read()
        img = Image.open(BytesIO(remove(raw))).convert("RGBA")
    except Exception:
    ...
    # Smart enhancement — boost clothes/objects, protect skin tones
    # Convert to HSV-like approach: only boost pixels that are NOT skin
    import numpy as np
    rgb_arr = np.array(rgb, dtype=np.float32)

    # Detect skin pixels: high R, medium G, low B
    ...
    enhanced_arr[skin_mask] = rgb_arr[skin_mask]  # revert skin to original
...
def build_snowfall_cycle(n_frames: int = CYCLE) -> List[Image.Image]:
    """
    Real 6-arm snowflakes. Seamless loop. Visible from frame 0.
    Speed increased. Fixed seed 42.
    """
    ...
    flakes = [
        {
            ...
            "speed": rng.uniform(18, 35),    # FASTER — was 5-12
    ...
    # Make seamless: speed * n_frames must be multiple of H
    for f in flakes:
...
def composite_frame(
    ...
    if avatar is not None:
        ...
        ay = AV_Y + avatar_y_off   # apply up/down offset
...
async def render_video(
    ...
    # Apply logo size override
    if logo_size > 0:
        import video_engine as _ve_self
        _orig_logo_h = LOGO_H
        # Temporarily override LOGO_H for this render
        import sys
        sys.modules[__name__].LOGO_H = logo_size
    ...
    # Restore LOGO_H
    if logo_size > 0:
        sys.modules[__name__].LOGO_H = _orig_logo_h
    ...
    cmd = [
        ...
        "-pix_fmt", "yuv420p", "-vf", f"scale={W}:{H}:flags=lanczos",
