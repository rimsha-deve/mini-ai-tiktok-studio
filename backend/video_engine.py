...
def load_font(size: int, font_name: str = "Anton") -> ImageFont.FreeTypeFont:
    ...
    FONT_MAP = {
        ...
        "Arial":          [r"C:\Windows\Fonts\arial.ttf", r"C:\Windows\Fonts\arialbd.ttf", "/usr/share/fonts/truetype/msttcorefonts/Arial.ttf", "Arial.ttf"],
...
def _stack_lines(text: str) -> List[str]:
    """
    Split text into lines for video rendering.
    PRIORITY: if text contains manual newlines (user pressed Enter in textarea),
    respect those EXACTLY.
    Otherwise: auto-split 2 words per line.
    """
    # Normalise — keep \n as explicit line breaks
    text = text.strip()

    # If user manually set line breaks, use them directly
    if '\n' in text:
        lines = [ln.upper().strip() for ln in text.split('\n') if ln.strip()]
        return lines
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 7 — NATURAL SNOW SYSTEM (3 types: pink, dust, night)
# ══════════════════════════════════════════════════════════════════════════════

def _draw_soft_circle(img: Image.Image, cx: int, cy: int, r: int,
                      color: Tuple, glow_r: int = 0):
    """Draw a soft glowing circle (natural snowflake/particle)."""
    if glow_r > 0:
        glow = Image.new("RGBA", (glow_r*2+2, glow_r*2+2), (0,0,0,0))
        for layer, (rf, af) in enumerate([(1.0,0.06),(0.65,0.12),(0.35,0.22)]):
            rd = int(glow_r*rf)
            ga = int(color[3]*af) if len(color)>3 else int(200*af)
            ImageDraw.Draw(glow).ellipse(
                [glow_r-rd, glow_r-rd, glow_r+rd, glow_r+rd],
                fill=(*color[:3], ga))
        gx, gy = cx-glow_r-1, cy-glow_r-1
        img.alpha_composite(glow, (max(0,gx), max(0,gy)))
    draw = ImageDraw.Draw(img)
    draw.ellipse([cx-r, cy-r, cx+r, cy+r], fill=color)


def _draw_hex_snowflake(img: Image.Image, cx: int, cy: int,
                         size: int, color: Tuple):
    """Draw a 6-arm natural snowflake (night snow style)."""
    draw = ImageDraw.Draw(img)
    alpha = color[3] if len(color) > 3 else 220
    arm_color  = (*color[:3], alpha)
    tick_color = (*color[:3], int(alpha * 0.65))

    for angle in range(0, 360, 60):
        rad = math.radians(angle)
        x2  = cx + size * math.cos(rad)
        y2  = cy + size * math.sin(rad)
        draw.line([(cx, cy), (x2, y2)], fill=arm_color, width=max(1, size//10))
        # Small cross-ticks at 60% of each arm
        for tick in [-35, 35]:
            t_rad = math.radians(angle + tick)
            tx    = cx + (size*0.6)*math.cos(rad)
            ty    = cy + (size*0.6)*math.sin(rad)
            tx2   = tx + (size*0.25)*math.cos(t_rad)
            ty2   = ty + (size*0.25)*math.sin(t_rad)
            draw.line([(tx, ty), (tx2, ty2)], fill=tick_color, width=max(1, size//14))

    # Center dot
    cr = max(1, size//6)
    draw.ellipse([cx-cr, cy-cr, cx+cr, cy+cr], fill=(*color[:3], min(255, alpha+20)))


def build_snow_layer(snow_type: str = "pink_snow",
                     n_frames: int = CYCLE,
                     density_pct: float = 1.0) -> List[Image.Image]:
    """
    Build one snow layer. Returns a seamless n_frames loop.

    snow_type:
      'pink_snow'  — soft pink/white circular particles (romantic feel)
      'snow_dust'  — tiny dense white dust (fine powder snow)
      'night_snow' — larger blue-white 6-arm hexagonal flakes (blizzard)

    density_pct: 1.0 = default density, 0.5 = half, 2.0 = double
    """
    rng = random.Random(SNOW_SEED)

    # ── Type configuration ───────────────────────────────────────
    if snow_type == "pink_snow":
        base_n     = int(40 * density_pct)
        size_range = (6, 20)
        spd_range  = (10, 22)
        drift_max  = 1.2
        alpha_range= (160, 210)
        ...
        def make_color(r):
            # Mix of white and pink
            if rng.random() > 0.45:
                pink = rng.randint(120, 200)
                return (255, pink, pink + rng.randint(10, 60), rng.randint(160, 210))
            return (255, 255, 255, rng.randint(170, 220))

        def draw_flake(img, x, y, size, color):
            _draw_soft_circle(img, x, y, size, color, glow_r=int(size*glow_mult))

    elif snow_type == "snow_dust":
        base_n     = int(90 * density_pct)
        size_range = (2, 7)
        spd_range  = (18, 40)
        drift_max  = 1.5
        alpha_range= (120, 190)
        ...
        def make_color(r):
            ...
            return (v, v, v, rng.randint(130, 195))

        def draw_flake(img, x, y, size, color):
            _draw_soft_circle(img, x, y, size, color, glow_r=int(size*glow_mult))

    else:  # night_snow
        base_n     = int(28 * density_pct)
        size_range = (10, 28)
        spd_range  = (6, 16)
        drift_max  = 0.8
        alpha_range= (150, 210)
        ...
        def make_color(r):
            blue = rng.randint(200, 255)
            return (200, 225, blue, rng.randint(150, 210))

        def draw_flake(img, x, y, size, color):
            _draw_soft_circle(img, x, y, size//3, color, glow_r=int(size*glow_mult))
            _draw_hex_snowflake(img, x, y, size, color)

    # ── Build flakes ─────────────────────────────────────────────
    flakes = []
    for i in range(base_n):
        sz = rng.randint(*size_range)
        flakes.append({
            "x":     rng.randint(0, W),
            "y":     int(i * H / base_n),   # evenly spread → visible from frame 0
            "size":  sz,
            "speed": rng.uniform(*spd_range),
            "drift": rng.uniform(-drift_max, drift_max),
            "color": make_color(sz),
            "phase": rng.uniform(0, math.pi*2),   # for opacity twinkle
            "twink": rng.uniform(0.06, 0.18),
        })

    # Seamless: speed × n_frames must be multiple of H
    for f in flakes:
        cycles   = max(1, round(f["speed"] * n_frames / H))
    ...
    for fi in range(n_frames):
        ...
        for f in flakes:
            cy = int((f["y"] + f["speed"] * fi) % H)
            cx = int((f["x"] + f["drift"]  * fi) % W)
            # Gentle twinkle
            twinkle = 0.82 + 0.18 * math.sin(f["phase"] + fi * f["twink"])
            r, g, b, a = f["color"]
            color = (r, g, b, int(a * twinkle))
            draw_flake(frame, cx, cy, f["size"], color)
        frames.append(frame)
    return frames


def build_snowfall_cycle(n_frames: int = CYCLE,
                          snow_types: List[str] = None,
                          density_pct: float = 1.0) -> List[Image.Image]:
    """
    Composite all enabled snow layers into one cycle.
    Default: all 3 types enabled (pink_snow + snow_dust + night_snow).
    """
    if snow_types is None:
        snow_types = ["pink_snow", "snow_dust", "night_snow"]

    if not snow_types:
        return [Image.new("RGBA", (W, H), (0,0,0,0)) for _ in range(n_frames)]

    # Build each type separately, then composite
    all_layers = [build_snow_layer(st, n_frames, density_pct) for st in snow_types]

    composited = []
    for fi in range(n_frames):
        frame = Image.new("RGBA", (W, H), (0,0,0,0))
        for layer in all_layers:
            frame.alpha_composite(layer[fi])
...
async def render_video(
    ...
    effects_cfg:       Optional[dict]  = None,   # effects config from frontend
) -> str:
    os.makedirs(TEMP_DIR, exist_ok=True)
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    # Defaults for effects config
    if effects_cfg is None:
        effects_cfg = {}
    snowfall_on = effects_cfg.get("snowfall", True)
    ...
    # ── 5. Snowfall ───────────────────────────────────────────────────────
    await prog("effects", 46, "Generating natural snow layers...")
    # Build snow with enabled types from config
    active_snow_types = []
    if effects_cfg.get("snow_pink",  True):  active_snow_types.append("pink_snow")
    if effects_cfg.get("snow_dust",  True):  active_snow_types.append("snow_dust")
    if effects_cfg.get("snow_night", True):  active_snow_types.append("night_snow")
    density = effects_cfg.get("snowfall_speed", 40) / 40.0  # 40% = 1.0x baseline

    snow_frames = await loop.run_in_executor(
        None, lambda: build_snowfall_cycle(
            snow_types=active_snow_types if snowfall_on else [],
            density_pct=max(0.3, min(3.0, density)),
        )
    )
