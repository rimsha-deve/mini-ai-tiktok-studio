...
export default function SourcesTab({ config, setConfig, avatarFile, setAvatarFile, bgFile, setBgFile, audioFile, setAudioFile }) {
  ...
  const handleAvatar = async f => {
    ...
    try {
      ...
      toast.success('Avatar uploaded! Detecting colors...');
      // Always fetch colors immediately after upload so preview matches video
      await fetchColors();
      toast.success('Colors detected! Preview updated ✓');
    } catch (e) {
