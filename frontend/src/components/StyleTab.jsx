...
const FONTS = [
  { name:'Anton',         preview:'ANTON',          style:{ fontFamily:'"Anton",Impact,sans-serif',                  fontWeight:400,  letterSpacing:'1px'  } },
  { name:'Impact',        preview:'IMPACT',         style:{ fontFamily:'Impact,"Arial Narrow",sans-serif',           fontWeight:400                        } },
  { name:'Arial',         preview:'ARIAL BOLD',     style:{ fontFamily:'Arial,Helvetica,sans-serif',                 fontWeight:800                        } },
  { name:'Bebas Neue',    preview:'BEBAS NEUE',     style:{ fontFamily:'"Bebas Neue",Impact,sans-serif',             fontWeight:400,  letterSpacing:'3px'  } },
  { name:'Bangers',       preview:'BANGERS',        style:{ fontFamily:'"Bangers",Impact,sans-serif',                fontWeight:400,  letterSpacing:'2px'  } },
  { name:'Oswald',        preview:'OSWALD',         style:{ fontFamily:'"Oswald","Arial Narrow",sans-serif',         fontWeight:700                        } },
  { name:'Montserrat',    preview:'MONTSERRAT',     style:{ fontFamily:'"Montserrat",Arial,sans-serif',              fontWeight:900                        } },
  { name:'Righteous',     preview:'Righteous',      style:{ fontFamily:'"Righteous",Arial,sans-serif',              fontWeight:400                        } },
  { name:'Pacifico',      preview:'Pacifico',       style:{ fontFamily:'"Pacifico",cursive',                        fontWeight:400                        } },
  { name:'Black Ops One', preview:'BLACK OPS ONE',  style:{ fontFamily:'"Black Ops One",Impact,sans-serif',         fontWeight:400,  letterSpacing:'1px'  } },
  { name:'Prohibition',   preview:'PROHIBITION',    style:{ fontFamily:'Impact,"Arial Narrow",sans-serif',           fontWeight:900,  letterSpacing:'4px'  } },
];
...
export default function StyleTab({ config, setConfig }) {
  ...
  const [logoStep,   setLogoStep]   = useState(40);

  const updT = (k, v) => setConfig(p => ({ ...p, text:   { ...p.text,   [k]: v } }));
  const updL = (k, v) => setConfig(p => ({ ...p, layout: { ...p.layout, [k]: v } }));

  const layout  = config.layout || {};
  const textX   = layout.text_x   ?? 0;
  const textY   = layout.text_y   ?? 0;
  const avatarX = layout.avatar_x >= 0 ? layout.avatar_x : 0;
  const avatarY = layout.avatar_y ?? 0;
  const logoX   = layout.logo_x   ?? 0;
  const logoY   = layout.logo_y   ?? 0;
    ...
    <div style={S.wrap}>
      ...
      <div>
        ...
        <div style={S.layoutCard}>
          ...
          {/* TikTok Logo arrow pad */}
          <ArrowPad
            label="TikTok Logo Position" icon="🎵"
            x={logoX} y={logoY}
            defaultX={0} defaultY={0}
            onX={v => updL('logo_x', v)}
            onY={v => updL('logo_y', v)}
            step={logoStep} setStep={setLogoStep}
          />
            ...
            <div style={S.slWrap}>
              ...
              <div style={S.slRow}>
                ...
                <input type="range" min={0} max={500}
                  ...
                  style={{ width:'100%', accentColor:'#FF2EAA' }} />
                <div style={S.slHints}><span>0 = Auto (220px)</span><span>500px max</span></div>
              </div>
