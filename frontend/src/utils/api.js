    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.status === 'processing') {
          ...
          // WS sends { status, data: { video_path, ... } } — unwrap data
...
export function connectQueueSocket(callbacks) {
  const ws = new WebSocket(`${WS_BASE}/queue`);
  ...
  ws.onmessage = (event) => {
    try {
      const data = JSON.parse(event.data);
      if (data.type === 'queue_state' || data.type === 'queue_update') {
        callbacks.onUpdate?.(data.jobs || []);
      } else if (data.type === 'job_complete') {
        callbacks.onJobComplete?.(data.job_id, data.result, data.jobs || []);
      } else if (data.type === 'job_error') {
        callbacks.onJobError?.(data.job_id, data.jobs || []);
      }
    } catch (e) {
...
export async function addToQueue(config, displayName = 'Video') {
  const response = await api.post('/queue/add', { config, display_name: displayName });
  return response.data;
}
...
export async function getBgMusicTracks() {
  const r = await api.get('/bgmusic/tracks');
  return r.data;
}

export async function downloadBgTrack(trackId) {
  const r = await api.post(`/bgmusic/download/${trackId}`);
  return r.data;
}
...
export async function uploadThumbnail(file) {
  ...
  fd.append('file', file);
  const r = await api.post('/upload/thumbnail', fd, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
  return r.data;
}
